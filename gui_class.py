...
class GUI(tk.Tk):
    ...
    def file_selector(self, label_text, starting_row)
        # student data (sd)
        sd_label = Label(window, text = label_text, font = ('bold', 12), padding='0.1i')
        sd_label.grid(sticky = "W", row = starting_row, column = 0)

        sd_button_frame = Frame(window)
        sd_button_frame.grid(row = starting_row, column = 1)

        sd_button = Button(sd_button_frame, text = "Browse", command = lambda: fileDialog(sd_location_label))
        sd_button.grid(row = starting_row, column = 0)
        sd_button = Button(sd_button_frame, text = "Clear", command = lambda: clear(sd_location_label))
        sd_button.grid(row = starting_row, column = 1)

        sd_location_label = Label(window, text = "")
        sd_location_label.grid(row = row_count + 1, column = 0, columnspan = 3)
    ...
    def __init__(self, *args, **kwargs): 
        ...
        # partition size (ps) selector
        ps_text = StringVar(window)
        ps_text.set(4)
        ps_label = Label(window, text = 'Number of Student Groups (2 or 4):', font = ('bold', 12), padding='0.1i')
        ps_label.grid(sticky = "W", row = 0, column = 0)
        ps_option_menu = OptionMenu(window, ps_text, 4, 2, 4)
        ps_option_menu.grid(row = 0, column = 1)

        # two groups (tg) size 
        tg_text = StringVar(window)
        tg_text.set(15)
        tg_label = Label(window, text = 'Max Class Size (2 Groups)', font = ('bold', 12), padding='0.1i')
        tg_label.grid(sticky = "W", row = 1, column = 0)
        tg_entry = Entry(window, textvariable = tg_text)
        tg_entry.grid(row = 1, column = 1)

        # four groups (fg) size 
        fg_text = StringVar(window)
        fg_text.set(9)
        fg_label = Label(window, text = 'Max Class Size (4 Groups)', font = ('bold', 12), padding='0.1i')
        fg_label.grid(sticky = "W", row = 2, column = 0)
        fg_entry = Entry(window, textvariable = fg_text)
        fg_entry.grid(row = 2, column = 1)

        # student data (sd)
        sd_label = Label(window, text = 'Student Course Data:', font = ('bold', 12), padding='0.1i')
        sd_label.grid(sticky = "W", row = 3, column = 0)

        sd_button_frame = Frame(window)
        sd_button_frame.grid(row = 3, column = 1)

        sd_button = Button(sd_button_frame, text = "Browse", command = lambda: fileDialog(sd_location_label))
        sd_button.grid(row = 0, column = 0)
        sd_button = Button(sd_button_frame, text = "Clear", command = lambda: clear(sd_location_label))
        sd_button.grid(row = 0, column = 1)

        sd_location_label = Label(window, text = "")
        sd_location_label.grid(row = 4, column = 0, columnspan = 3)


        # required student subgroups (rs)
        rs_label = Label(window, text = 'Required Student Subgroups:', font = ('bold', 12), padding='0.1i')
        rs_label.grid(sticky = "W", row = 5, column = 0)

        rs_button_frame = Frame(window)
        rs_button_frame.grid(row = 5, column = 1)

        rs_button = Button(rs_button_frame, text = "Browse", command = lambda: fileDialog(rs_location_label))
        rs_button.grid(row = 0, column = 0)
        rs_button = Button(rs_button_frame, text = "Clear", command = lambda: clear(rs_location_label))
        rs_button.grid(row = 0, column = 1)

        rs_location_label = Label(window, text = "")
        rs_location_label.grid(row = 6, column = 0, columnspan = 3)


        # preferred student subgroups (psub)
        psub_label = Label(window, text = 'Preferred Student Subgroups:', font = ('bold', 12), padding='0.1i')
        psub_label.grid(sticky = "W", row = 7, column = 0)

        psub_button_frame = Frame(window)
        psub_button_frame.grid(row = 7, column = 1)

        psub_button = Button(psub_button_frame, text = "Browse", command = lambda: fileDialog(psub_location_label))
        psub_button.grid(row = 0, column = 0)
        psub_button = Button(psub_button_frame, text = "Clear", command = lambda: clear(psub_location_label))
        psub_button.grid(row = 0, column = 1)

        psub_location_label = Label(window, text = "")
        psub_location_label.grid(row = 8, column = 0, columnspan = 2)

        # time limit (tl)
        tl_text = StringVar(window)
        tl_text.set(480)
        tl_label = Label(window, text = 'Runtime Limit (Minutes)', font = ('bold', 12), padding='0.1i')
        tl_label.grid(sticky = "W", row = 9, column = 0)
    ...
    def fileDialog(label):
        filename = filedialog.askopenfilename(initialdir =  "IO_DIRECTORY", title = "Select A File", filetype = (("csv","*.csv"),("all files","*.*")) )
        label.configure(text = filename)

    def clear(label):
        label.configure(text="")

    def launch(widget_list):
        NUMBER_OF_PARTITIONS = int(ps_text.get())
        HALF_CLASS_MAXIMUM = int(tg_text.get())
        QUARTER_CLASS_MAXIMUM = int(fg_text.get())
        TIME_LIMIT = int(tl_text.get())
        
        INPUT_CSV_FILENAME = sd_location_label["text"]
        if len(INPUT_CSV_FILENAME) == 0:
            INPUT_CSV_FILENAME = None
        
        REQUIRED_SUBGROUP_CSV_FILENAME = rs_location_label["text"]
        if len(REQUIRED_SUBGROUP_CSV_FILENAME) == 0:
            REQUIRED_SUBGROUP_CSV_FILENAME = None
        
        PREFERRED_SUBGROUP_CSV_FILENAME = psub_location_label["text"]
        if len(PREFERRED_SUBGROUP_CSV_FILENAME) == 0:
